                            ...
                            for (let key in row) {
                                ...
                                if (key === 'Réponse') {
                                    switch (row[key].charCodeAt(0)) {
                                        case 65 :
                                            question.answer = !!question.options[0] ? question.options[0].value : 'A' ;
                                            break;
                                        case 66 :
                                            question.answer = !!question.options[1] ? question.options[1].value : 'B';
                                            break;
                                        case 67 :
                                            question.answer = !!question.options[2] ? question.options[2].value : 'C';
                                            break;
                                        case 68 :
                                            question.answer = !!question.options[3] ? question.options[3].value : 'D';
                                            break;
                                        default:
                                            break;
                                    }
...
function saveQuizToDataBase(quiz) {
    return new Promise((resolve, reject) => {
        if ((!!quiz.grade || quiz.grades.length > 0) && !!quiz.points && !!quiz.subject) {
            quiz.save();
            console.log('Quiz saved');
            resolve(quiz);
        } else {
            if (quiz.grades.length === 0 && !!quiz.grade) {
                console.log('No grade found for this quiz');
            } else if (!!quiz.points) {
                console.log('No points set for this quiz');
            } else if (!!quiz.subject) {
                console.log('No subject found for this quiz');
            }
        }
...
function addOptions() {
    Quiz.find({saveOptions: {$exists: true}, subject: {$exists: false}, grade: {$exists: false}}, (err, quiz) => {
        ...
        if (!quiz || quiz.length === 0) {
            ...
            quiz.map(async (q) => {
                try {
                    console.log(q.saveOptions.subject.trim(), ' and ',q.saveOptions.grade.trim() );
                    q.subject = await getSubjectId(q.saveOptions.subject);
                    let grade = await getGrade(q.saveOptions.grade);
                    q.grade = grade;
                    q.grades.push(grade);
                    ...
                    console.log('Error adding options');
                }
            });
        }
    });
...
function getSubjectId(subj) {
    return new Promise((resolve, reject) => {
        return Subject.findOne({'name.en': subj.trim()}, (err, subject) => {
            if (err || subject === null) reject("Error no subject");
            if (subject) {
                resolve(subject._id);
            }
        });
...
function getGrade(gr) {
    return new Promise((resolve, reject) => {
        return Grade.findOne({category: gr.trim()}, (err, grade) => {
            if (err || !grade) {
                reject("Error no grade");
            }
            if (!!grade) {
                resolve(grade._id);
            }
